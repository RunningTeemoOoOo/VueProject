...
export default {
  ...
  mounted () {
    ...
    axios.get(`http://www.xiongmaoyouxuan.com/api/search?word=${this.word}start=${this.current}&sort=${this.sort}&couponOnly=NaN&minPrice=0&maxPrice=99999`).then(res => {
  ...
  methods: {
    ...
    loadMore () {
      this.loading = true
      this.current += 20
      axios.get(`http://www.xiongmaoyouxuan.com/api/search?word=%E5%8F%A3%E7%BA%A2&start=${this.current}&sort=${this.sort}&couponOnly=NaN&minPrice=0&maxPrice=99999`).then(res => {
        console.log(res.data)
